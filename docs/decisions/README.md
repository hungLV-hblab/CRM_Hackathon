...
| --- | --- | --- | --- |
| [0047](0047-contract-phan-trang-offset-co-khoa-phu.md) | Design | **Phân trang offset dùng chung** `{items,total,page,pageSize}`; `ORDER BY` **bắt buộc có khoá phụ `id`** vì `created_at` không unique và vòng quét chạy 10s/lần; **cấm `z.coerce.boolean()`** — nó là `Boolean(input)` nên `"false"` thành `true`. Đính chính tiền đề sai của bản brainstorm: hai danh sách này **không** tự tăng vô hạn | Chấp nhận — đang thi công |
| [0046](0046-phan-quyen-theo-nguoi-phu-trach-toan-he-thong.md) | Requirement / Design | **Phân quyền theo `companies.owner_id` cho toàn bộ đường đọc lẫn đường ghi.** Thay thế **một phần** ADR-0033 (điều kiện xem lại mà chính nó nêu — "seed có từ hai người sở hữu trở lên" — đã xảy ra) và lật **phương án E** của ADR-0045. Bịt lỗ ghi chéo chủ sở hữu qua `undo()`. Cổng **fail-closed**, một tầng và nói thật là một tầng | Chấp nhận — đang thi công |
| [0045](0045-dang-nhap-demo-tai-dung-luong-mat-khau-va-dashboard-scope-theo-vai.md) | Design / Development | Đăng nhập demo **tái dùng luồng mật khẩu** (không mở endpoint bỏ qua mật khẩu); dashboard scope theo vai, sở hữu đi qua công ty; người phụ trách lấy từ cột `sales_owner` của dữ liệu BTC | Chấp nhận — **phương án E bị lật bởi ADR-0046** |
| [0044](0044-kiem-tra-claude-code-chay-that-di-qua-api-chu-khong-mo-them-cua-cong-khai.md) | Development | **"Có credential" không phải "chạy được", nên phải chạy thật một lượt mới được nói là đạt**: `resolveAuthMode()` chỉ kiểm sự tồn tại, nên credential hết hạn · token bị thu hồi · hết quota · thiếu binary đều hiện badge xanh như nhau. Lượt kiểm tra đi qua **`POST /api/settings/agent-check` (admin) → `/run/health-check` bằng `Bearer AGENT_TOKEN`**; route ticket trên runtime bị loại vì nó đặt một endpoint **tiêu quota** vào đúng họ route Caddy forward ra ngoài. Prompt là **skill dữ liệu** `allowedTools: []` (soi được ở `/health.grants`), không phải hằng số TS. Ghi `lastRun` cho **mọi** lượt `/run/*` — nút bấm chỉ ép một lượt xảy ra, không phải nguồn sự thật. Bộ nhớ tiến trình, không CSDL: container mới thì thật sự chưa ai kiểm tra nó. Mỗi `AgentFailureReason` ra một câu **kèm việc phải làm** — hết credential bảo đăng nhập lại, hết quota bảo **đừng** bấm lại | Chấp nhận — 622 test xanh (600 → 622) + 3/3 e2e panel; **gọi thật `agent-check` trên stack: 200, model trả "OK", 6154ms, 16.084 token vào**, `lastRun.authMode` báo đúng credential đã chạy. Sales → 403, `/run` từ ngoài → 307, Caddyfile không đổi dòng nào. **Chưa verify:** các nhánh hỏng mới khoá bằng unit test, chưa gây ra được trên stack thật |
| [0043](0043-dang-nhap-claude-qua-giao-dien-va-vi-sao-api-chi-ky-ve.md) | Development | **Đăng nhập Claude bấm được từ `/quan-tri`, và `api` chỉ ký vé chứ không đứng giữa**: trình duyệt POST **thẳng** tới `agent-runtime` qua tiền tố `/agent-auth/*` của Caddy, nên tiến trình cầm `DATABASE_URL_SYSTEM` không bao giờ thấy mã uỷ quyền lẫn credential (ADR-0038 nguyên vẹn). Vé HMAC 5 phút dùng-một-lần **ký bằng `AGENT_TOKEN`** — không đẻ credential mới; đưa thẳng `AGENT_TOKEN` cho trình duyệt bị loại vì mất luôn cửa `/run`. PTY bằng `util-linux`/`script` chứ không `node-pty` (image thiếu toolchain). Bóc URL bằng **tiền tố giao thức**, không bằng câu tiếng Anh. `resolveAuthMode()` không sửa dòng nào | Chấp nhận — 600 test xanh (532 → 600); `/run` vẫn **không** với tới được từ `:8080`. **Lỗi nặng nhất do người dùng thật tìm ra:** Ink coi mỗi chunk stdin là một sự kiện phím, nên ghi `code+"\n"` một lần là chèn text chứ không phải bấm Enter → treo hết 5' trong khi test cũ assert đúng hành vi sai đó. Sửa: Enter là lần ghi RIÊNG; +dò `OAuth error:` (0,35s thay vì 5'); +giết cả nhóm tiến trình & `init: true` (0 zombie). Review trước đó tìm thêm 3 lỗi đường-thất-bại đã sửa; **chưa verify** bước bấm đồng ý trên trang Anthropic (cần người) |
